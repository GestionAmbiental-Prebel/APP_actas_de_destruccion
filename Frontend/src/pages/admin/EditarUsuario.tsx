...
export const EditarUsuario = () => {
  ...
  // Simulación: aquí iría el fetch al backend para obtener los datos
  useEffect(() => {
    // fetch(`/api/usuarios/${id}`)
    //   .then(res => res.json())
    //   .then(data => setFormData(data));
    console.log("Cargar datos de usuario con ID:", id);
    setFormData({
      nombre: "Juan Pérez",
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // 🔹 Aquí iría la lógica para hacer PUT/PATCH al backend
  };
  ...
  return (
    <div className="p-6 font-acidGrotesk max-w-2xl mx-auto">
      ...
      >
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            Contraseña (dejar vacío si no se desea cambiar)
          </label>
          <input
            type="password"
            name="password"
